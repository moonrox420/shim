...
class GitAutoCommitHandler(FileSystemEventHandler):
    ...
    def trigger_commit(self, relative_path: str) -> None:
        try:
            ...
            print(
                f"[{datetime.now(timezone.utc).strftime('%H:%M:%S')}] Auto-staging: {relative_path}"
            )
    ...
    def on_any_event(self, event: FileSystemEvent) -> None:
        ...
        if any(pattern in src_path for pattern in ignored):
            return

        current_time = time.time()
        relative_path = os.path.relpath(src_path, start=self.repo_path)
