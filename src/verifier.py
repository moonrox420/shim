...
try:
    from contract import AgentContract, Proof, IMPLEMENTER_V1, generate_property_test_stub
except ImportError:
    from .contract import AgentContract, Proof, IMPLEMENTER_V1, generate_property_test_stub
...
def verify_basic(language: Optional[str], cwd: Path) -> List[Dict[str, Any]]:
    ...
    if language in ("rust", "rs"):
        ...
        if "No module named ruff" in (out or ""):
            results.append({"name": "ruff check", "passed": True, "output": "skipped (ruff not installed)", "skipped": True})
        else:
            results.append({"name": "ruff check", "passed": ok, "output": out})

        rc2, out2 = _run_cmd_with_rc([sys.executable, "-m", "pytest", "-q", "--tb=line"], cwd, timeout=120)
        if "No module named pytest" in (out2 or ""):
            results.append({"name": "pytest (best effort)", "passed": True, "output": "skipped (pytest not installed)", "skipped": True})
        elif rc2 == 5:
            results.append({"name": "pytest (best effort)", "passed": True, "output": "skipped (no tests collected)", "skipped": True})
        else:
            results.append({"name": "pytest (best effort)", "passed": rc2 == 0, "output": out2})
    else:
        results.append({"name": "language check", "passed": True, "output": f"no specific verifier for {language}"})
    return results
...
def verify_implementation(
    *,
    ...
    ignore_names = {".git", "__pycache__", ".venv", "venv", "node_modules", "target", "dist", "build", ".pytest_cache", ".ruff_cache"}
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if extra_commands:
            for cmd in extra_commands:
                ...
                verifier_results.append({"name": " ".join(cmd), "passed": ok, "output": out})

    real_results = [r for r in verifier_results if not r.get("skipped")]
    overall = all(r.get("passed", False) for r in real_results) if real_results else True
    ...
    if not uncertain:
        ...
        for r in verifier_results:
            ...
            for sig in ("traceback", "error:", "failed", "exception", "syntaxerror", "fatal"):
                if sig in out:
                    bad_signals += 1
    ...
    if overall and not uncertain and language in ("python", "py"):
        hint = next((str(vr.get("output", ""))[:200] for vr in verifier_results if "new logic" in (vr.get("output") or "").lower() or "added" in (vr.get("output") or "").lower()), "")
        if hint:
            stub = generate_property_test_stub(hint, language)
            if stub:
                touched = set((changed_files or {}).keys())
                has_test_file = any(p.endswith(".py") and ("test" in p.lower() or p.startswith("tests/")) for p in touched)
...
if __name__ == "__main__":
    ...
    proof = verify_implementation(changed_files={"dummy.py": "print('hi')\n"}, language="python")
